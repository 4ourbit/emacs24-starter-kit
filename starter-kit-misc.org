...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Cycle buffers in current window with ibuffer-bs. Also once in selection
  ;; buffer pass through repeated C-M-o. Note that "C-M-S-o" is bound to
  ;; reverse cycling in ibuffer-mode (ivy: minibuffer-i-mode) in its major
  ;; mode key map.
  ...
  (define-key my/keys-minor-mode-map (kbd "C-u C-o")
    `(menu-item "" window-swap-states
                :filter ,(lambda (cmd)
                           (unless (derived-mode-p 'minibuffer-inactive-mode) cmd))))
